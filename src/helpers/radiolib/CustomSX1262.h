...
public:
...
#endif
  {
...
#ifdef LORA_RADIO_BOOT_POWER
    const int8_t boot_power = LORA_RADIO_BOOT_POWER;
#else
    const int8_t boot_power = LORA_TX_POWER;
#endif
    ...
    int status = begin(LORA_FREQ, LORA_BW, LORA_SF, cr, RADIOLIB_SX126X_SYNC_WORD_PRIVATE,
                       boot_power, 16, tcxo, useRegulatorLDO);
    if (status == RADIOLIB_ERR_SPI_CMD_FAILED || status == RADIOLIB_ERR_SPI_CMD_INVALID) {
      MESH_DEBUG_PRINTLN("SX1262 init failed with error %d, retrying with TCXO at 0.0V", status);
      tcxo = 0.0f;
      status = begin(LORA_FREQ, LORA_BW, LORA_SF, cr, RADIOLIB_SX126X_SYNC_WORD_PRIVATE,
                     boot_power, 16, tcxo, useRegulatorLDO);
    }
...
#define SX126X_TXEN RADIOLIB_NC
  ...
  bool isReceiving()
  {
    ...
    const bool preamble = irq & RADIOLIB_SX126X_IRQ_PREAMBLE_DETECTED;
    const bool header = irq & RADIOLIB_SX126X_IRQ_HEADER_VALID;
    const bool hdrErr = irq & RADIOLIB_SX126X_IRQ_HEADER_ERR;
