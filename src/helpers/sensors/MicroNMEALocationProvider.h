...
public:
  MicroNMEALocationProvider(Stream &ser, mesh::RTCClock *clock = NULL,
                            int pin_reset = GPS_RESET, int pin_en = GPS_EN,
  ...
  void loop() override
  {
    ...
    if (static_cast<long>(millis() - next_check) > 0) {
